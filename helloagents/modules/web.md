...
### Summary History（Replay，可回放基础）
**条件**: 用户在 RAG Debugger 的「Show summary history (replay)」展开后查看  
**行为**: 调用 `/v1/ops/condensations?namespace=...&session_id=...` 拉取该 session 的 condensation 列表  
**结果**: 可查看不同 `version` 的结构化卡片与触发元信息（便于面试解释“版本化/回放”）
